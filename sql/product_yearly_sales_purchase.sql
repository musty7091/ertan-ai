...
SatisHam AS (
    ...
    INNER JOIN Urun u ON (h.STOKNO = u.StokInd OR h.BARKOD = u.Barkod)
    WHERE b.TARIH >= '2026-01-01' AND b.TARIH < '2027-01-01'
      AND b.IND <> 166576
      AND LTRIM(RTRIM(b.FTIPI)) IN ('FIS', 'FATURA', 'G.PUSULA')
),
...
AlisHam AS (
    SELECT
        ...
        CASE WHEN ISNULL(b.IADE, 0) = 1 THEN -(h.GERCEKTOPLAM + ISNULL(h.KDVTUTARI, 0)) ELSE (h.GERCEKTOPLAM + ISNULL(h.KDVTUTARI, 0)) END AS AlisKdvDahil,
        CASE
    ...
    SELECT
        ...
        CASE WHEN ISNULL(b.IADE, 0) = 1 THEN -(h.GERCEKTOPLAM + ISNULL(h.KDVTUTARI, 0)) ELSE (h.GERCEKTOPLAM + ISNULL(h.KDVTUTARI, 0)) END,
        CASE
            WHEN ISNULL(b.IADE, 0) = 0 AND h.MIKTAR > 0 AND (
                ISNULL(h.GERCEKTOPLAM, 0) = 0
                OR ISNULL(h.ISK1, 0) = 100 OR ISNULL(h.ISK2, 0) = 100 OR ISNULL(h.ISK3, 0) = 100
