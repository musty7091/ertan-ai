...
def run_category_profit(conn, category: str, limit: int = 50) -> pd.DataFrame:
    ...
    return query_df(conn, sql, [category, limit])
...
def render_category_profit(df: pd.DataFrame, category: str):
    ...
    st.subheader(f"🏷️ {category} - 2026 Kategori Kârlılık Raporu")
    ...
    st.info("Kategori raporu 2026 satış ve alış faturası verilerine göre hesaplanır. Brüt kâr KDV hariç mantıkla gösterilir.")
